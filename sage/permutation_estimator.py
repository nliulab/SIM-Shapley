...
class PermutationEstimator:
    ...
    def __call__(
        ...
        if return_all:
            return core.Explanation(tracker.values, tracker.std, explanation_type), record_list
        else:
    ...
    def _process_sample(self, x, y, num_features, min_coalition, max_coalition):
        ...
        # permutations: batch_size X num_features. permutations[0,:] = [0, 1, 2, 3,....,num_features-1],每一行都相同

        # Sample permutations.
        for i in range(batch_size):
            self.rng.shuffle(permutations[i]) # 每行shuffle
